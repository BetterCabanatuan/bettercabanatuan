    ...
    <>
      ...
      <section className="relative overflow-hidden bg-gradient-to-r from-primary-600 to-primary-700 text-white py-16 md:py-24">
        <div className="container mx-auto px-4 text-center">
          {/*
              A decorative watermark, not content: the <h1> below already says
              what happened, so this is hidden from assistive tech rather than
              read out as "four zero four" ahead of the real heading.

              It still has to be *visible*, though. At `text-white/20` it
              measured 1.54:1 against the light end of the gradient — a
              large-text graphic needs 3:1, and a "404" nobody can see is just
              a rendering artefact. `text-white/60` clears it at 3.58:1.
            */}
          <p
            className="text-8xl font-bold text-white/60 mb-2"
            aria-hidden="true"
          >
            404
          </p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {[
              ...
              <Link
                key={link.href}
                to={link.href}
                className="flex items-center min-h-[44px] px-4 py-3 rounded-lg border border-gray-200 text-sm font-medium text-gray-800 hover:border-primary-300 hover:text-primary-700 hover:bg-primary-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-primary-600 transition-[background-color,border-color,color] duration-200"
              >
                {link.label}
              </Link>
            ))}

...
const overviewStats = [
  {
    ...
    // Year + program read from city-data.json so this can never drift from
    // the homepage or /about.
    note: `${cityStats.populationReference.year} Census (${cityStats.populationReference.program})`,
    icon: Users,
        ...
        <div className="container mx-auto px-4 relative">
          <div className="motion-safe:animate-fade-in mb-8">
            ...
            <Text className="text-white/85 max-w-2xl text-base md:text-lg mb-0">
              ...
              public data for {siteConfig.governmentName}, {siteConfig.province}
              .
            </Text>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
            {overviewStats.map((stat, index) => (
              <Card
                key={stat.label}
                hoverable
                className="h-full ring-1 ring-black/[0.06] motion-safe:animate-stat-rise hover:-translate-y-1 transition-transform duration-300"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              {services.map((service, index) => (
                ...
                >
                  <Link
                    ...
                    className="flex items-center justify-between min-h-[44px] px-4 py-2 rounded-lg border border-gray-200 text-sm font-medium text-gray-800 hover:border-primary-300 hover:text-primary-700 hover:bg-primary-50 hover:translate-x-0.5 transition-[transform,background-color,border-color,color] duration-200"
                  >
                    {service.category}
          ...
          >
            ...
            <Link
              ...
              className="inline-flex items-center gap-2 min-h-[44px] px-5 py-2.5 bg-primary-600 text-white rounded-lg font-medium hover:bg-primary-700 hover:gap-3 focus:outline-none focus-visible:ring-2 focus-visible:ring-primary-600 focus-visible:ring-offset-2 transition-[gap,background-color] duration-200 motion-safe:animate-fade-in"
